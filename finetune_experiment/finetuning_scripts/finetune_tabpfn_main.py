...
def fine_tune_tabpfn(
    *,
    ...
    small_improvement_threshold: float = 0.005,
    small_improvement_patience: int = 5,
    # Finetuning HPs
    time_limit: int,
    ...
    # === Early Stopping based on small improvements ===
    #improvement_threshold = 0.005
    #max_small_improve_steps = 5
    ...
    for step_i, batch_data in iter_steps_pbar:
        ...
        if validate_now:
            ...
            if is_best:
                ...
                logger.info(f"✅ Saved best model at step {step_i} with val_loss = {validation_loss:.4f}")
                torch.save(
                    dict(
                        state_dict=model.module.state_dict() if is_data_parallel else model.state_dict(),
                        config=checkpoint_config),
            ...
            # === Early Stopping by Small Improvement ===
            if last_val_loss is not None:
                improvement = abs(last_val_loss - validation_loss)
                if improvement < small_improvement_threshold:
                    small_improve_count += 1
                else:
                    small_improve_count = 0
            else:
                small_improve_count = 0  # 初始化

            last_val_loss = validation_loss

            if small_improve_count >= small_improvement_patience:
                logger.warning(
                    f"[Early Stop] Validation loss improvement < {small_improvement_threshold} "
                    f"for {small_improvement_patience} consecutive validations. Stopping..."
                )
                break
        else:
